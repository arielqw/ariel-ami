...
exec(char *path, char **argv)
{
	...
	thread->process->isPendingExec = 1;
	thread->process->threadThatCalledExec = thread;

    //kill all threads but self
	int numOfOtherThreads = 0;
	struct thread* t;
	for(t = thread->process->ttable.thread; t < &thread->process->ttable.thread[NTHREAD]; t++){
		if(t != thread){
			t->killed = 1; // kill all but current thread
			if(t->state != UNUSED && t->state != ZOMBIE) numOfOtherThreads++;
		}
	...
	cprintf("%d | %s starting exec code (pid %d) \n", thread->tid, LOG_TAG, thread->process->pid);
	  ...
	  for(last=s=path; *s; s++)
	    if(*s == '/')
	      last = s+1;
	  safestrcpy(thread->process->name, last, sizeof(thread->process->name));

	  // Commit to the user image.
	  oldpgdir = thread->process->pgdir;
	  thread->process->pgdir = pgdir;
